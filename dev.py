...
def train_movement_1(data_df, 
    ...
    max_features = cf["model"]["movement_1"]["max_features"]
    thresh_hold = cf["training"]["movement_1"]["corr_thresh_hold"]
    output_step = cf["model"]["movement_1"]["output_steps"]
    train_df = utils.prepare_dataset_and_indicators(train_df, window_size)
    valid_df = utils.prepare_dataset_and_indicators(valid_df, window_size)
    test_df = utils.prepare_dataset_and_indicators(test_df, window_size)
    ...
    if is_train:
        train.train_Movement_1(dataset_train_trend, dataset_val_trend, full_features)
    infer.evalute_Movement_1(dataset_val=dataset_val_trend, features = full_features)
    infer.evalute_Movement_1(dataset_val=dataset_test_trend, features = full_features)

def train_magnitude_1(data_df, 
                    num_data_points,
                    train_df, valid_df,
                    test_df, train_date,valid_date, test_date,
                    data_dates, show_heat_map = False, is_train = False):
    ...
    max_features = cf["model"]["magnitude_1"]["max_features"]
    thresh_hold = cf["training"]["magnitude_1"]["corr_thresh_hold"]
    output_step = cf["model"]["magnitude_1"]["output_steps"]
    train_df = utils.prepare_dataset_and_indicators(train_df, window_size)
    valid_df = utils.prepare_dataset_and_indicators(valid_df, window_size)
    test_df = utils.prepare_dataset_and_indicators(test_df, window_size)
    ...
    if is_train:
        train.train_magnitude_1(dataset_train_trend, dataset_val_trend, full_features)
