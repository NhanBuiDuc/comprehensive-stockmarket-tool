...
class Unified_Adversarial_Loss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        target_cls_int = torch.argmax(target[:, :2], dim=1)  # Convert the one-hot encoding to integers
        output_cls_int = torch.argmax(output[:, :2], dim=1)  # Convert the one-hot encoding to integers
